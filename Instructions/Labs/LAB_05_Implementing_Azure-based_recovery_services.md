...
1. **SEA-SVR2** 上の、Azure portal で、次の設定を使用して他の仮想ネットワークを作成します (他の設定は既定値のままにします)。
...
1. 新しい仮想ネットワーク内で、次の設定を使用してサブネットを作成します (他の設定は既定値のままにします)。
   ...
   > **注:**  Azure Site Recovery では、仮想マシンのレプリケートに既定でマネージド ディスクが使用されるようになりました。 レプリケーション ログとキャッシュ データを保存するために、Azure Site Recovery によってキャッシュ ストレージ アカウントが自動的に作成されます。 これにより、VM ディスク レプリケーション用のストレージ アカウントを手動で作成する必要がなくなります。
...
1. Azure portal を表示しているブラウザー ページを更新し、**[1. インフラストラクチャの準備]** 手順の最初のステップを繰り返します。
...
1. **[レプリケーションの有効化]** ブレードの **[ターゲット環境]** タブで、次の設定を指定します (他の設定は既定値のままにします)。
   ...
   > **注:**  Azure Site Recovery により、仮想マシンのレプリケートにマネージド ディスクが自動的に使用されます。 必要に応じて、キャッシュ ストレージ アカウントが自動的に作成されます。
