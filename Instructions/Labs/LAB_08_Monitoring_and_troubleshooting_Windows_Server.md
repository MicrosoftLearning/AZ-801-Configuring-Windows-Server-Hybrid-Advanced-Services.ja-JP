...
Contoso, Ltd. は、米国ワシントン州シアトルに本社を置くグローバルなエンジニアリングおよび製造企業です。 IT オフィスとデータセンターはシアトルにあり、シアトルの場所やその他の場所をサポートしています。 Contoso は最近、Windows サーバーサーバーとクライアント インフラストラクチャをデプロイしました。
...
仮想マシン: **AZ-801T00A-SEA-DC1** と **AZ-801T00A-SEA-SVR2** が実行されている必要があります。 他の VM は実行できますが、このラボでは必要ありません。
...
> **注**: データ コレクター セットを開始した後、結果が表示されるまでに 10 分の遅延が発生する場合があります。
