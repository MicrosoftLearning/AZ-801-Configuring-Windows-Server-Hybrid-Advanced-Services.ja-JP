...
1. **[コードの入力]** テキスト ボックスに、クリップボードにコピーしたコードを貼り付け、 **[次へ]** をクリック します。
1. **[サインイン]** ページで、前の演習で Azure サブスクリプションへのサインインに使用したものと同じユーザー名を入力し、 **[次へ]** をクリックします。次に、対応するパスワードを入力し、 **[サインイン]** をクリックします。
1. 「**Windows Admin Center にサインインしますか?** 」 というメッセージが表示されたら、 **[続行]** をクリックします。
...
1. **[Azure Monitor のセットアップ]** ページで、次の設定を指定し、 **[セットアップ]** をクリックします。
   ...
   >**注**: 設定が完了するのを待たずに、次の演習に進んでください。 セットアップには約 3 分かかります。
...
1. **SEA-SVR2** で、Azure portal のツールバーにある **[リソース、サービス、ドキュメントの検索]** テキスト ボックスで「**モニター**」を検索して選択します。次に **[モニター\| 概要]** ページの **[Insights]** で **[仮想マシン]** を選択します。
...
1. **[アラート ルールの作成]** ページの **[アクション]** タブで **[アクション グループの追加]** を選択し、 **[+ アクション グループの作成]** をクリックします。
...
1. **[新しいクエリ 1]** タブで **[テーブル]** ヘッダーを選択し、 **[仮想マシン]** セクションのテーブルの一覧を確認します。
   ...
   >**注**: いくつかのテーブルの名前は、このラボで先ほどインストールしたソリューションに対応しています。 特に、**InsightMetrics** は、パフォーマンス メトリックを格納するために Azure VM Insights で使用されます。
