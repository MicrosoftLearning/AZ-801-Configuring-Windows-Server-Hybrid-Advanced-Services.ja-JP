...
この演習の所要時間は約 **120** 分です。 <!-- update with estimated duration -->
...
1. Azure portal のツールバーの **[リソース、サービス、ドキュメントの検索]** テキスト ボックスで、**[Azure Migrate]** を検索して選択し、**[Azure Migrate \| はじめに]** ページで、**[プロジェクトの作成]** を選択します。
...
1. **[Azure Migrate | はじめに]** ページで、左側のメニューの **[すべてのプロジェクト]** を選択し、**[az801l07a-migrate-project]** プロジェクトを選択します。
1. **[az801l07a-migrate-project \| 概要]** ページで、**[検出の開始]** を選択し、**[アプライアンスの使用]** を選択し、**[Azure 用]** を選択します。
1. **[検出]** ページの **[サーバーは仮想化されていますか?]** ドロップダウン リストで、**[はい。Hyper-V を使用します]** を選択します。 
...
1. 仮想マシンのメモリ サイズを **[4096 MB]** に設定し、**[動的メモリの有効化]** チェック ボックスをオフにします。
...
1. **[Appliance Configuration Manager]** ページの **[最新の更新プログラムを確認し、アプライアンスを登録する]** セクションで、**[こちらにキーを貼り付けて Hyper-V アプライアンスを登録します]** テキスト ボックスに、この演習で先にメモ帳コピーしたキーを貼り付け、**[確認]** を選択します。 
1. **[Azure ユーザー ログインとアプライアンスの登録状態]** セクションで、**[ログイン]** を選択します。 **[Azure ログインの続行]** ウィンドウで、**[コードのコピーとログイン]** を選択します。 これにより、コピーしたコードの入力を求める新しいブラウザー タブが自動的に開きます。
...
1. **[Appliance Configuration Manager]** ページの **[ステップ 3: インストールされているソフトウェア、依存関係、ワークロードのゲスト検出を実行するためのサーバー資格情報を指定する]** セクションで **[ゲスト検出を既定で無効にする]** トグル ボタンを**無効に**し、**[検出の開始]** を選択します。

   >**注**: 検出されたサーバーのメタデータが Azure portal に表示されるまでに、ホストごとに約 15 分かかります。

#### タスク 2: 評価を構成、実行、表示する

1. 仮想アプライアンスへの **[仮想マシン接続]** ウィンドウから、**az801l07a-hv-vm** へのリモート デスクトップ セッションに切り替えます。 Azure portal が表示されているブラウザー ウィンドウで、**[Azure Migrate | az801l07a-migrate-project]** に戻り、**[最新の情報に更新]** を選択します。 **[概要]** タブの **[評価]** で、**[評価の作成]** を選択します。
1. **[評価の作成]** ページの **[基本情報]** タブで、**[評価名]** として「**az801l07a-assessment**」と入力し、**[ワークロードの追加]** を選択します。
1. **[ワークロードの選択]** ページで、**[az801l07a-vm1]** チェック ボックスを選択して、**[追加]** を選択し、**[次へ]** を選択します。   
1. **[評価の作成]** ページの **[基本情報]** タブで次の設定を指定し (他の設定は既定値のままにしておきます)、**[次へ]** を選択します。
   ...
   | 既定の保存オプション | **なし** |
   | 割引 | **0** |
   | Uptime | 1 か月あたり **31** 日、1 日あたり **24** 時間 | 
   | サイズ変更の設定基準 | **オンプレミス** |
   | 快適性係数 | **1** |

   >**注**: ラボ環境に固有の限定された時間を考慮すると、ここで唯一実行可能なオプションは、**[オンプレミス]** 評価です。 

1. **[詳細]** タブの **[インフラストラクチャ設定]** で、**[既定値の編集]** を選択します。
1. **[Azure VM]** が **[ターゲット サービス]** の下で選択されていることを確認し、**[既定値の編集]** を選択し、次のパラメーターを指定して **[保存]** を選択します。

   | 設定 | Value | 
   | --- | --- |
   | VM シリーズ | **Dsv4_series** |
   | ストレージの種類 | **Premium マネージド ディスク** |
   | VM のセキュリティの種類 | **Standard** |

1. **[サーバー (マシン) の設定]** ページで、**[保存]** を選択します。
1. **[評価の確認と作成]** を選択し、**[作成]** を選択します
...
1. **[az801l07a-hv-vm]** へのリモート デスクトップ セッション内の Azure portal が表示されているブラウザー ウィンドウで、**[Azure Migrate | az801l07a-migrate-project]** ページに戻ります。 
1. **[Azure Migrate | az801l07a-migrate-project]** ページで、左側のメニュー タブの **[実行]** の下にある **[移行]** を選択します。
1. **[移行のための検出]** の下にある **[その他]** を選択します。

1. **[検出]** ページで、次の設定を指定して **[リソースの作成]** を選択します。
...
1. **[検出]** ページが表示されているブラウザー ウィンドウを最新の情報に更新します。 ページの最初の 2 つの値を指定すると、**[登録の終了処理]** ボタンが表示されます。 **[登録の終了処理]** を選択します。
1. 登録が完了したことを確認するまで待ちます。
...
1. **[az801l07a-hv-vm]** へのリモート デスクトップ セッション内の Azure portal が表示されているブラウザー ウィンドウで、**[Azure Migrate | az801l07a-migrate-project]** ページに戻ります。 
1. **[Azure Migrate | az801l07a-migrate-project]** ページで、左側のメニュー タブの **[実行]** の下にある **[移行]** を選択します。
1. ページ上部の **[レプリケート]** ボタンを選択します。
1. **[意図の指定 | レプリケート]** ページの **[お使いのマシンは仮想化されていますか?]** ドロップダウン リストで、**[はい。Hyper-V を使用します]** を選んでから **[続行]** を選択します。
...
1. **[レプリケート]** ページの **[ターゲットの設定]** タブで、次の設定を指定し (他の設定は既定値のまま)、**[次へ]** を選択します。
   ...
   | サブスクリプション | このラボで使用している Azure サブスクリプションの名前 |
   | リソース グループ | **AZ801-L0703-RG** |
   | キャッシュ ストレージ アカウント | このラボで前に作成したストレージ アカウントの名前 | 
...
1. レプリケーションの状態を監視するため、**[az801l07a-migrate-project | 移行]** に戻り、**[レプリケーションの要約]** を選択し、**[移行]** セクションで **[レプリケーション]** エントリを選択します。 **[Azure Migrate: サーバーの移行 | レプリケーション]** ページで、レプリケート中のマシンの一覧の **[レプリケーションの状態]** 列を確認します。
1. 状態が **[保護されている]** に変わるまで待ちます。 これには、さらに 15 分かかることがあります。

   >**注**:**[Azure Migrate: サーバーの移行 |レプリケーション]** を更新して **[状態]** 情報を更新する必要があります。

#### タスク 3: Hyper-V VM の移行を実行する

1. Azure portal の **[Azure Migrate: サーバーの移行 | レプリケーション]** ページで、**az801l07a-vm1** 仮想マシンを表すエントリを選択します。
...
1. Azure portal で、**[Azure Migrate: サーバーの移行 | レプリケーション]** ページに戻り、**[最新の情報に更新]** を選択して、**az801l07a-vm1** 仮想マシンが **[テスト フェールオーバーのクリーンアップが保留中]** 状態で一覧表示されていることを確認します。
1. **[Azure Migrate: サーバーの移行 | レプリケーション]** ページで、**az801l07a-vm1** 仮想マシンを表すエントリを選択します。
...
1. **[移行]** ページで、 **[仮想マシンをシャットダウンし、データ損失のない計画された移行を実行しますか?]** ドロップダウン リストで **[はい]** が選択されていることを確かめてから、 **[移行]** を選びます。
1. 移行の状態を監視するため、**[az801l07a-migrate-project | 移行]** ページに戻ります。 **[レプリケーションの概要]** を選択し、**[レプリケーション]** エントリを選択してから、**[Azure Migrate: サーバーの移行 | レプリケーション]** ページで、レプリケート中のマシンの一覧の **[レプリケーションの状態]** 列を確認します。 状態に **[計画フェールオーバーが完了]** が表示されていることを確認します。

   >**注**: 移行は、元に戻すことができないアクションであることが想定されています。 完全な情報を確認する場合は、**[Azure Migrate: サーバーの移行]** ページに戻り、**[移行]** セクションの **[移行されたサーバー]** エントリの値が **1** であることを確認します。
