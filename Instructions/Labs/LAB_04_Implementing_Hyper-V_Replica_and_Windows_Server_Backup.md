...
1. 前のタスクで作成したネットワーク共有を使用して **SEA-SVR2** でバックアップの場所を構成するには、**SEA-SVR2** で、**SEA-SVR1** への PowerShell リモート処理セッションを使用して、次のコマンドを実行します (サインインを求めるメッセージが表示されたら、講師から提供された資格情報を入力します)。
